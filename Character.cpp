...
operator==
*/
bool Character::operator==(const Character &r) const {
    return (name_ == r.name_  && race_ == r.race_ && level_ == r.level_ && enemy_ == r.enemy_) ;
}
...
bool Character::operator!=(const Character &r) const {
    if (name_ != r.getName()  && race_ != r.race_ && level_ != r.getLevel() && enemy_ != r.isEnemy()) {
        return true;
...
void Character::display() const{
    ...
    std::cout <<"Max Armor: " <<armor_ <<"\n";
    std::cout<< (enemy_ ? "They are an Enemy.\n" : "They are not an enemy.\n");
}
