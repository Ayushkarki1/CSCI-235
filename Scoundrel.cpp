...
Project 4 Task 1- Task 1: Modify the Character class and its Subclasses (Scoundrel)
...
void Scoundrel::display() const {
    ...
    std::cout << "Disguise: " << (has_disguise_ ? "TRUE" : "FALSE") << std::endl << std::endl;
}
...
void Scoundrel::eatTaintedStew() {
    if (getRace() == "UNDEAD") {
        ...
        if (getFaction() == "CUTPURSE") {
            // Steal a health potion
            setVitality(getVitality() + 3);
        } else if (getFaction() == "SILVERTONGUE") {
            // 70% chance of recovering 4 vitality, 30% chance of resetting to 1
            int chance = rand() % 100;
            if (chance < 70) {
                setVitality(std::min(4, getVitality() + 4));
            } else {
                setVitality(1);
